  ...
  return (
    <section>
      <AdminPanelHeader
        ...
        action={<Button variant="outline" size="sm" onClick={resource.reload} aria-label="刷新用户列表"><RefreshCw />刷新</Button>}
      />
      ...
      {resource.loading ? <AdminLoading /> : resource.error ? <AdminError message={resource.error} onRetry={resource.reload} retryLabel="刷新用户列表" /> : null}
              ...
              <tbody>
                {resource.data.items.map(user => (
                  <tr key={user.id} className="border-t border-border/60">
                    <td className="p-3"><div className="flex items-center gap-2"><span className="font-medium">{user.nickname || user.username}</span>{user.is_superuser ? <Badge variant="outline"><ShieldCheck className="mr-1 h-3 w-3" />管理员</Badge> : null}</div><div className="mt-1 text-xs text-muted-foreground">{user.id}</div></td>
                    ...
                    <td className="pr-3 text-right"><Button variant="ghost" size="sm" onClick={() => void openUser(user.id)}>查看详情</Button></td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {selectedUser ? (
        <aside className="mt-4 rounded-xl border border-border bg-card p-4">
          <h2 className="font-semibold">{selectedUser.nickname || selectedUser.username}</h2>
          <dl className="mt-3 grid gap-3 text-sm sm:grid-cols-2"><div><dt className="text-xs text-muted-foreground">完整邮箱</dt><dd>{selectedUser.email || '—'}</dd></div><div><dt className="text-xs text-muted-foreground">注册时间</dt><dd>{formatAdminDate(selectedUser.created_at)}</dd></div></dl>
